...
cdef class job:
	...
	def get(self):
		...
		:returns: Data where key is the job id, each entry contains a dictionary of job attributes
		:rtype: `dict`
		"""

		print ("job get before load")

		self.__load()
		print ("job get after load")

		self.__toPythonObject()
		print ("job get after pythonObject")


		return self._JobDict

	cpdef __toPythonObject(self):
		...
		:rtype: `dict`
		"""
		print ("__toPythonObject start")

		cdef:
			int i = 0
			dict Jobs = {}
			jobInfo auxJob = jobInfo()

		print ("__toPythonObject before loop")


		if self._job_ptr is not NULL:
			print ("__toPythonObject not null")
			for i from 0 <= i < self._job_ptr.record_count:
				print ("__toPythonObject inside loop 1. i value is " + str(i))
				self._record = self._job_ptr.job_array[i]
				print ("__toPythonObject inside loop 2")
				job_id = self._job_ptr.job_array[i].job_id
				print ("__toPythonObject inside loop 3. job id value is " + str(job_id))
				Jobs[job_id] = auxJob.get(job_id)
				print ("__toPythonObject inside loop 4")
...
cdef class jobInfo:
	...
	u"""Class to access/modify Slurm Job Information.
		Note that this is for a single job, while the previous class is for all the jobs on the system 
	"""

	cdef:
		slurm.slurm_job_info_t *_job  #pointer to the job  (internal data structure)
		slurm.time_t _lastUpdate
		uint16_t _ShowFlags

		dict _JobInfo #same information than  _job, but converted to Python object for manipulation
	...
	def id(self):
		...
		u"""Return the job ID from retrieved data.

		:returns: job ID
		:rtype: `int`
		"""

		return self._JobDict['id']
	...
	cpdef int __load(self, id):
		...
		if id == -1:
			return -1  #TODO these error return values sould be specified somewhere, I guess...
		...
		last_time = <time_t>NULL
		errCode = slurm.slurm_load_job(&new_job_ptr, id, self._ShowFlags)
		if errCode == 0:
			slurm.slurm_free_job_info_msg(new_job_ptr)  #NOT sure of this at all
		if errCode == 1900: # NO CHANGE IN DATA
			return 1900


		if errCode == 0:

			if new_job_ptr.record_count == 0:
				return 1 #TODO these error return values sould be specified somewhere, I guess...
			elif new_job_ptr.record_count > 1:
				return 2

			self._job = &new_job_ptr.job_array[0]
			self._lastUpdate = self._job_ptr.last_update
	...
	def get(self, id = -1):
		...
		print ("JobInfo get- loading info from job " + str(id))
		self.__load(id)
		print ("JobInfo get- converting info into python object ")
	...
	cpdef __toPythonObject(self):
		...
		if self._job is not NULL:
			...
			Job_dict = {}
			Job_dict[u'id'] = job_id
			Job_dict[u'account'] = slurm.stringOrNone(self._job.account, '')
			Job_dict[u'alloc_node'] = slurm.stringOrNone(self._job.alloc_node, '')
			Job_dict[u'alloc_sid'] = self._job.alloc_sid
			Job_dict[u'array_job_id'] = self._job.array_job_id
			Job_dict[u'array_task_id'] = self._job.array_task_id
			Job_dict[u'array_max_tasks'] = self._job.array_max_tasks
			Job_dict[u'array_task_str'] = slurm.stringOrNone(self._job.array_task_str, '')
			Job_dict[u'assoc_id'] = self._job.assoc_id
			Job_dict[u'batch_flag'] = self._job.batch_flag
			Job_dict[u'batch_host'] = slurm.stringOrNone(self._job.batch_host, '')
			Job_dict[u'batch_script'] = slurm.stringOrNone(self._job.batch_script, '')
			Job_dict[u'boards_per_node'] = self._job.boards_per_node
			Job_dict[u'command'] = slurm.stringOrNone(self._job.command, '')
			Job_dict[u'comment'] = slurm.stringOrNone(self._job.comment, '')
			Job_dict[u'contiguous'] = bool(self._job.contiguous)
			Job_dict[u'core_spec'] = self._job.core_spec
			Job_dict[u'cores_per_socket'] = self._job.cores_per_socket
			Job_dict[u'cpus_per_task'] = self._job.cpus_per_task
			Job_dict[u'dependency'] = slurm.stringOrNone(self._job.dependency, '')
			Job_dict[u'derived_ec'] = self._job.derived_ec
			Job_dict[u'eligible_time'] = self._job.eligible_time
			Job_dict[u'end_time'] = self._job.end_time
			Job_dict[u'exc_nodes'] = slurm.listOrNone(self._job.exc_nodes, ',')
			Job_dict[u'exit_code'] = self._job.exit_code
			Job_dict[u'features'] = slurm.listOrNone(self._job.features, ',')
			Job_dict[u'gres'] = slurm.listOrNone(self._job.gres, ',')
			Job_dict[u'group_id'] = self._job.group_id

			# JOB RESOURCES HERE

			#Job_dict[u'job_state'] = get_job_state(self._job.job_state)
			Job_dict[u'job_state'] = slurm.slurm_job_state_string(self._job.job_state)
			Job_dict[u'licenses'] = __get_licenses(self._job.licenses)
			Job_dict[u'max_cpus'] = self._job.max_cpus
			Job_dict[u'max_nodes'] = self._job.max_nodes
			Job_dict[u'name'] = slurm.stringOrNone(self._job.name, '')
			Job_dict[u'network'] = slurm.stringOrNone(self._job.network, '')
			Job_dict[u'nodes'] = slurm.listOrNone(self._job.nodes, ',')
			Job_dict[u'nice'] = self._job.nice

			#if self._job.node_inx[0] != -1:
			#	for x from 0 <= x < self._job.num_nodes

			Job_dict[u'ntasks_per_core'] = self._job.ntasks_per_core
			Job_dict[u'ntasks_per_node'] = self._job.ntasks_per_node
			Job_dict[u'ntasks_per_socket'] = self._job.ntasks_per_socket
			Job_dict[u'ntasks_per_board'] = self._job.ntasks_per_board
			Job_dict[u'num_nodes'] = self._job.num_nodes
			Job_dict[u'num_cpus'] = self._job.num_cpus
			Job_dict[u'partition'] = self._job.partition
			Job_dict[u'pn_min_memory'] = self._job.pn_min_memory
			Job_dict[u'pn_min_cpus'] = self._job.pn_min_cpus
			Job_dict[u'pn_min_tmp_disk'] = self._job.pn_min_tmp_disk
			Job_dict[u'preempt_time'] = self._job.preempt_time
			Job_dict[u'pre_sus_time'] = self._job.pre_sus_time
			Job_dict[u'priority'] = self._job.priority
			Job_dict[u'profile'] = self._job.profile
			Job_dict[u'qos'] = slurm.stringOrNone(self._job.qos, '')
			Job_dict[u'reboot'] = self.__get_select_jobinfo(SELECT_JOBDATA_REBOOT)
			Job_dict[u'req_nodes'] = slurm.listOrNone(self._job.req_nodes, ',')
			Job_dict[u'req_switch'] = self._job.req_switch
			Job_dict[u'requeue'] = bool(self._job.requeue)
			Job_dict[u'resize_time'] = self._job.resize_time
			Job_dict[u'restart_cnt'] = self._job.restart_cnt
			Job_dict[u'resv_name'] = slurm.stringOrNone(self._job.resv_name, '')

			Job_dict[u'sockets_per_board'] = self._job.sockets_per_board
			Job_dict[u'sockets_per_node'] = self._job.sockets_per_node
			Job_dict[u'shared'] = self._job.shared
			Job_dict[u'show_flags'] = self._job.show_flags
			Job_dict[u'start_time'] = self._job.start_time
			Job_dict[u'state_desc'] = slurm.stringOrNone(self._job.state_desc, '')
			#Job_dict[u'state_reason'] = get_job_state_reason(self._job.state_reason)
			Job_dict[u'state_reason'] = slurm.slurm_job_reason_string(self._job.state_reason)
			Job_dict[u'std_err'] = slurm.stringOrNone(self._job.std_err, '')
			Job_dict[u'std_in'] = slurm.stringOrNone(self._job.std_in, '')
			Job_dict[u'std_out'] = slurm.stringOrNone(self._job.std_out, '')
			Job_dict[u'submit_time'] = self._job.submit_time
			Job_dict[u'suspend_time'] = self._job.suspend_time
			Job_dict[u'time_limit'] = self._job.time_limit
			Job_dict[u'time_min'] = self._job.time_min
			Job_dict[u'threads_per_core'] = self._job.threads_per_core
			Job_dict[u'user_id'] = self._job.user_id
			Job_dict[u'wait4switch'] = self._job.wait4switch
			Job_dict[u'wckey'] = slurm.stringOrNone(self._job.wckey, '')
			Job_dict[u'work_dir'] = slurm.stringOrNone(self._job.work_dir, '')
	...
	cpdef __get_select_jobinfo(self, uint32_t dataType):
		...
		cdef:
			slurm.dynamic_plugin_data_t *jobinfo = <slurm.dynamic_plugin_data_t*>self._job.select_jobinfo
	...
	def print_job_info_msg(self, int oneLiner=0):
		...
		if self._job is not NULL:
			slurm.slurm_print_job_info(slurm.stdout, self._job, oneLiner)
